...
//    descriptor->compute( rgb2, kp2, desp2 );
    ...
    // 尽管经过了筛选，我们提供的匹配里还是存在误匹配的情况。根据误匹配计算运动是不靠谱的。
    // “随机采样一致性”（Random Sample Consensus）
    // 在现有的匹配中随机取一部分，估计其运动。因为正确的匹配结果肯定是相似的，而误匹配的结果肯定满天乱飞。只要把收敛的结果取出来即可。
    // ransac适用于数据噪声比较大的场合
    // 通过调节我们之前的筛选过程以及之后的ransac参数，得到更好的结果。
