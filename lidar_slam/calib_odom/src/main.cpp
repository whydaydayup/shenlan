...
void Scan2::publishPathEigen(std::vector<Eigen::Vector3d>& path_eigen,ros::Publisher& path_pub_)
{
    ...
    tmpPose.header.frame_id="odom"; // 设置frame_id为odom
...
//TODO:
//求解得到两帧数据之间的位姿差,相当于位姿增量
//即求解当前位姿　在　上一时刻　坐标系中的坐标
// odom_pose是机器人在世界坐标系中的实时的位姿量
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)// 每次里程计只有一个数据odom_pose
{
    ...
    double x_last = last_pos(0);
    double y_last = last_pos(1);
    double theta_last = last_pos(2);

    Eigen::Matrix3d Tlast;
    Tlast << cos(theta_last), -sin(theta_last),    x_last,
             sin(theta_last),  cos(theta_last),    y_last,
                           0,                0,         1;


    double x_now = now_pos(0);
    double y_now = now_pos(1);
    double theta_now = now_pos(2);

    Eigen::Matrix3d Tnow;
    Tnow << cos(theta_now), -sin(theta_now),    x_now,
            sin(theta_now),  cos(theta_now),    y_now,
                         0,               0,        1;

    // Tij = Ti(-1) * Tj
    Eigen::Matrix3d T = Tlast.inverse() * Tnow;
...
int main(int argc,char** argv)
{
    ...
    // bag的数据超过12000了,要是在12000之前进行矫正就不会报错
    // 在12000之前矫正的话,后面的数据就不会用了,
    // 可以将12000改大,或加一个判断12000后面的数据不要了,或12000之后的数据替代前面的,或在12000之前就进行标定都可以
