...
public:
    ...
    */
            ...
            int& beam_number)
    {
        ...
        {
            double x_now = ranges[i+startIndex]*cos(angles[i+startIndex]);
            double y_now = ranges[i+startIndex]*sin(angles[i+startIndex]);
            double aaaa = i;

            tf::Vector3 aa = frame_start_pose.getOrigin().lerp(frame_end_pose.getOrigin(), aaaa/(beam_number-1));
            tf::Quaternion bbb = frame_start_pose.getRotation().slerp(frame_end_pose.getRotation(), aaaa/(beam_number-1));

            tf::Transform T1(bbb, aa); // 插值法后获得的当前的T
            tf::Transform Tb(frame_base_pose.getRotation(), frame_base_pose.getOrigin());//base_pose的T
            tf::Transform Tend = Tb.inverse() * T1;

            double x_T = Tend.getOrigin().getX(); // x
            double y_T = Tend.getOrigin().getY(); // y
            double yaw_T = tf::getYaw(Tend.getRotation()); // theta

            double x_end = x_now * cos(yaw_T) - sin(yaw_T) * y_now + x_T;
            double y_end = x_now * sin(yaw_T) + cos(yaw_T) * y_now + y_T;

            angles[i+startIndex] = atan2(y_end, x_end);
            ranges[i+startIndex] = sqrt(x_end * x_end + y_end * y_end);
            /* code */
        }
