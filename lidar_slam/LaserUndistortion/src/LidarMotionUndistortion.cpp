...
public:
    ...
    void ScanCallBack(const champion_nav_msgs::ChampionNavLaserScanPtr& scan_msg)
    {
        ...
        g_PointCloudView.runOnVisualizationThreadOnce (viewerOneOff);
    ...
    */
            ...
            int& beam_number)
    {
        ...
        {
            // 当前激光点的range、angles转换成px，py
            double x_distort = ranges[i+startIndex]*cos(angles[i+startIndex]); //未畸变点的xy坐标
            double y_distort = ranges[i+startIndex]*sin(angles[i+startIndex]);

            // 第i点激光时，车辆在里程计坐标系下的位置x, y, z
            tf::Vector3 frame_i_translation = frame_start_pose.getOrigin().lerp(frame_end_pose.getOrigin(), i/(beam_number-1));
            // 第i点激光时，车辆在里程计坐标系下的姿态（旋转），
            tf::Quaternion frame_i_rotation = frame_start_pose.getRotation().slerp(frame_end_pose.getRotation(), i/(beam_number-1));
            // 第i点激光时，车辆在里程计坐标系下的位姿（即相对于里程计坐标系原点的旋转+平移，转换矩阵）
            tf::Transform frame_i_transform(frame_i_rotation, frame_i_translation);

            // 基准坐标系相对于里程计坐标系原点的转换矩阵（旋转+平移）
            tf::Transform frame_base_transform(frame_base_pose.getRotation(), frame_base_pose.getOrigin());

            // 基准坐标系转换矩阵-1 * 激光i时车辆的位姿 == 激光i时车辆在基准坐标系中的位姿
            tf::Transform laser_base_transform = frame_base_transform.inverse() * frame_i_transform;

            // (px' , py' ,1) = T * (px, py, 1)
            // T = cos -sin x
            //     sin  cos y
            //       0    0 1
            tfScalar x = laser_base_transform.getOrigin().getX();
            tfScalar y = laser_base_transform.getOrigin().getY();
            tfScalar theta = tf::getYaw(laser_base_transform.getRotation());

            tf::Matrix3x3 T = tf::Matrix3x3(cos(theta), -sin(theta), x,
                                            sin(theta),  cos(theta), y,
                                                     0,           0, 1);
            tf::Vector3 v_distort(x_distort, y_distort, 1);
            tf::Vector3 v_undistort = T * v_distort;

            // range = sqrt(x^2 + y^2), angle = arctan(y/x)
            tfScalar x_undistort = v_undistort.getX();
            tfScalar y_undistort = v_undistort.getY();

            angles[i+startIndex] = atan2(y_undistort, x_undistort);
            ranges[i+startIndex] = sqrt(x_undistort * x_undistort + y_undistort * y_undistort);
            /* code */
        }
       //end of TODO
    }
