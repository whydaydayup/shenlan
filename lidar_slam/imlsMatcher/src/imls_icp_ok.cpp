...
//        Wi = exp(-pow(tempVi.norm(),2)/ (m_h * m_h));
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    for(std::vector<Eigen::Vector2d>::iterator it = nearPoints.begin(); it != nearPoints.end(); ++it)
    {
        Eigen::Vector2d T1 = (*it - mean); // 2x1
        Eigen::RowVector2d T2 = T1; // 1x2
        Eigen::Matrix2d tmp = T1 * T2; // 2x2
    ...
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(variance, Eigen::ComputeThinU | Eigen::ComputeThinU);
    // 特征值一般按照降序排列
    ...
    Eigen::VectorXd tmp = U.col(i); // 相应的列
    normal = tmp;

    // 特征值一般按照降序排列,所有可直接等于第1列(从第0列开始)
    //normal = U.col(1);
    cout << normal << endl;
    //end of TODO
