...
{
    // 创建一个vector中存储Vector2d表示源点云
    std::vector<Eigen::Vector2d> source_ptCloud;
    // 遍历整个pcl点云,将每个点云的x,y坐标保存到Vector2d中
...
// 设置源点云的重载形式,读入存储了Eigen::Vector2d类型的点云x,y坐标的vector容器,
...
{
    // 创建一个vector中存储Vector2d表示target目标点云
...
void IMLSICPMatcher::setTargetPointCloud(std::vector<Eigen::Vector2d> &_target_pcloud)
{
    ...
    // 若目标点云的KD-Tree不为空,删除这个KD-Tree,并将指针指向nullptr空指针
    if(m_pTargetKDTree != NULL)
    {
        delete m_pTargetKDTree;
    ...
    if(m_pTargetKDTree == NULL)
    {
        ...
        // 创建一个m_targetKDTreeDataBase的KD-Tree, note that m_targetKDTreeDataBase must stay valid during the lifetime of the kd-tree.
        m_pTargetKDTree = Nabo::NNSearchD::createKDTreeLinearHeap(m_targetKDTreeDataBase);
    }

    //设置需要重新计算法向量(设置flag为false)
                                         ...
                                         double& height)
{
    ...
    // 找到位于点x附近(m_r)的所有的点云
    int searchNumber = 20; // 查找点x周围最近的20个nearest neighbours
    Eigen::VectorXi nearIndices(searchNumber); // VectorXi保存
    ...
    //最大搜索距离为m_r
    m_pTargetKDTree->knn(x, nearIndices, nearDist2, searchNumber, // query points, indices, dists2, number of nearest neighbour requested
                         0, // 不需要近似值, 0.1表示按照10%的近似值, epsilon, maximal ratio of error for approximate search
                         Nabo::NNSearchD::SORT_RESULTS // 按距离进行排序SORT_RESULTS
                         | Nabo::NNSearchD::ALLOW_SELF_MATCH| // allow self-match.
                         Nabo::NNSearchD::TOUCH_STATISTICS,
                         m_r); // 最大邻近搜索半径m_r, 用来精简搜索prune search, 不被epsilon参数影响

    std::vector<Eigen::Vector2d> nearPoints; // 保存所有最近邻的点云坐标x,y
    std::vector<Eigen::Vector2d> nearNormals;// 保存所有最近邻的点云对应的法向量
    for(int i = 0; i < searchNumber;i++)
    {
        //说明最近邻是合法的．
        // 距离的平方 < infinity()正无穷 && 不是 inf && 不是nan
                ...
                std::isnan(nearDist2(i)) == false)
        {
            ...
            normal = m_targetPtCloudNormals[index]; // 提取点对应的法向量

            //如果对应的点没有法向量，则不要．
            if(std::isinf(normal(0))||std::isinf(normal(1))||
                    std::isnan(normal(0))||std::isnan(normal(1)))
            {
                // 若其中一个数据是非法数据，跳出当前循环，执行下一次循环，即舍弃当前数据
                continue;
            }

            // std::vector<Eigen::Vector2d> nearPoints;
            // std::vector<Eigen::Vector2d> nearNormals;
            nearPoints.push_back(tmpPt); // 最近领的点云坐标系x,y
            nearNormals.push_back(normal); // 最近邻的点云对应的法向量
        }
        else
        {
            break;
    ...
    for(int i = 0 ; i < nearPoints.size();i++)
    {
        ...
        // 计算距离邻近点pi到点x的距离
        // 两个Eigen::Vector2d相减,然后norm计算二范数,即sqrt(x^2 + y^2)欧式距离
    ...
    // 投影点的高度, 即点x到曲面的距离, x的所有最近邻点的距离*权重/权重的总和
...
{
    out_cloud.clear(); // clear()删除容器内所有的数据
    out_normal.clear();
...
/** NICP中的某一个点pi所对应的法向量,这个点周围的所有的点呈高斯分布,来计算这个点对应的法向量
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    RemoveNANandINFData(nearPoints);
    //TODO

    // 初始化为零矩阵
    Eigen::Vector2d mean = Eigen::Vector2d::Zero();; // 均值
    Eigen::Matrix2d variance = Eigen::Matrix2d::Zero();; // 方差
    // 求均值 ui = sum(pi) / N
    for(std::vector<Eigen::Vector2d>::iterator it = nearPoints.begin();it != nearPoints.end(); ++it)
    {
        mean += *it;
    }
    mean = mean/nearPoints.size();

    // 求方差 (pi - ui)T * (pi-ui) / N 得到2*2的方差矩阵
    // 迭代一定要改变变量的条件++it
    for(std::vector<Eigen::Vector2d>::iterator it = nearPoints.begin();it != nearPoints.end(); ++it)
    {
        Eigen::Vector2d T1 = (*it - mean);
        Eigen::RowVector2d T2 = T1;
        Eigen::Matrix2d tmp = T1 * T2;
        // Eigen::Matrix2d tmp = (*it - mean) * (*it - mean).inverse();// 直接处理出错
        variance += tmp;
    }
    variance = variance/(nearPoints.size() -1);

    Eigen::JacobiSVD<Eigen::Matrix2d> svd(variance, Eigen::ComputeThinU | Eigen::ComputeThinU);
    // 特征值一般按照降序排列
    Eigen::VectorXd SVector = svd.singularValues(); // singular values特征值的向量
    Eigen::MatrixXd U = svd.matrixU();

    ptrdiff_t i;
    SVector.minCoeff(&i); // 最小值所在的行数
    normal = U.col(i); // 相应的列
    //end of TODO
                           ...
                           Eigen::Matrix3d& covariance)
{
    ...
    result.setIdentity(); // setIdentity()设为单位阵
    covariance.setIdentity();

    for(int i = 0; i < m_Iterations;i++) // m_Iterations迭代次数
    {
        //根据当前估计的位姿对原始点云进行转换．
        std::vector<Eigen::Vector2d> in_cloud;
        // std::vector<Eigen::Vector2d> m_sourcePointCloud 遍历源点云
        for(int ix = 0; ix < m_sourcePointCloud.size();ix++)
        {
            Eigen::Vector3d origin_pt;
            origin_pt << m_sourcePointCloud[ix], 1;

            // 现在的位姿 3x1 = 估计值result 3x3 * [x, y, 1] 3x1
        ...
        // 最后得到的匹配的点云的数量,若小于5则报错,并退出当前循环大的for循环
        ...
        Eigen::Matrix3d deltaTrans; // 微小的位姿差
